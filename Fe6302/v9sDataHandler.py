...
def SingleStokes(datapath, stokes, slicenumber, basepath='/Users/jhamer/Research/Data'):
    """NAME: SingleStokes
...
def Spectrum(datapath, stokes, slicenumber, xpos, basepath='/Users/jhamer/Research/Data'):
    """NAME: Spectrum
...
def LoadedDataSpectrum(Islicedata, slicedata, xpos):
    """NAME: Spectrum
    ...
    for i in range(len(slicedata)):
        Iflux.append(Islicedata[i][xpos])
        flux.append(slicedata[i][xpos])
    if np.mean(Iflux[97:115])>0:
        flux=flux/np.mean(Iflux[97:115])
    return flux
