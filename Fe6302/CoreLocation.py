...
def LocChecker(datapath, row, col, deg=2, line='Iron', basepath='/Users/jhamer/Research/Data'):
    """NAME: LocChecker 
    ...
    """
    flux=v9s.Spectrum(datapath=datapath, stokes='I', slicenumber=v9s.SliceNumber(row), xpos=col, basepath=basepath)
    if line=='Iron':
    ...
    loc=np.abs(xs[-1]-xs[0])*(minloc/1000)+xs[0]
    fig=plt.figure()
    plt.plot(flux, linewidth=2, label='Data')
    plt.plot(x, p(x), linewidth=2, label='Polynomial Fit')
    plt.scatter(loc, p(loc), label='Polynomial Minimum')
    plt.xlabel('Wavelength Pixel')
    plt.ylabel('Intensity')
    plt.title("Row: "+str(row)+", Col: "+str(col)+' Core Location Demo')
...
def IndexMaps(datapath, array):
    ...
    """
    ironmap=np.zeros((len(array), len(array[0])))
    oxygen1map=np.zeros((len(array), len(array[0])))
    oxygen2map=np.zeros((len(array), len(array[0])))
    ...
    for i in range(len(array)):
        if solarboolean[i]==True:
            slicedata=v9s.SingleStokes(datapath=datapath, stokes='I', slicenumber=i)
            (limb1, limb2)=limbs[i]
            j=limb1
            while(j<gapcols[0]):
                PixelIndices(slicedata, ironmap, i, j, line='Iron')
                PixelIndices(slicedata, oxygen1map, i, j, line='Oxygen1')
                PixelIndices(slicedata, oxygen2map, i, j, line='Oxygen2')
                j=j+1
            j=gapcols[1]
            while(j<limb2):
                PixelIndices(slicedata, ironmap, i, j, line='Iron')
